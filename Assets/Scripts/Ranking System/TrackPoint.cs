...
//        IconManager.SetIcon(trackPoint, IconManager.Icon.DiamondPurple);
    ...
    void Update()
    {
        ...
        RankManager.instance.SetRank(new Runner()
        {
    ...
    {
       // lineStart = WayPointManager.instance.waypoints[activeWaypointIndex].position;

        lineEnd = lineStart;
        if (activeWaypointIndex < WayPointManager.instance.waypoints.Count - 1)
            lineEnd = WayPointManager.instance.waypoints[activeWaypointIndex + 1].position;
